...
* [태그](../components/tag), [processor](../components/processor) 를 통해 생성된 HTML 요소를 템플릿화하여, 다른 형태의 look & feel 로 전환할 수 있도록 함
...
내장 theme 는 현재 `proto` 만 존재하며, 별도의 설정이 없을 경우 이 theme 로 구동됩니다. 

{% alerts style="info" %}
`jekyll-potion` 의 테마는 향후 계속적으로 추가될 예정입니다.
{% endalerts %}

custom theme 는 사용자가 직접 마크업 작업을 하여, 정해진 규칙에 맞게 설정하면, 이를 구동 시킬 수 있도록 합니다. 자세한 내용은 [theme 만들기](../components/theme/make-theme) 를 참고하세요.
...
| `theme[].assets.source_dir`       | string          |  -  | _assets    | theme 내 asset 들의 기본 경로로서 theme path 기준의 상대 경로                                                                                                                                      |
...
| `theme[].content_x_path`          | string          |  -  | _main      | rendering 된 HTML 중 본문 영역에 해당되는 [Nokogiri](https://github.com/sparklemotion/nokogiri) Node에서 접근 가능한 css selector 값<br/>검색 및 자체 내장 javascript에서 사용                                   |

## processor 설정

processor 로 인해 생성되는 마크업 요소의 설정값을 다룹니다. processor 별 자세한 설명은 [processor](../components/processor) 를 통해 확인할 수 있으며 세부 항목은 아래와 같습니다.
...
태그로 인해 생성되는 마크업 요소에 대한 설정값입니다. 태그에 대한 자세한 설명은 [태그](../components/tag) 에서 다루며, 상세한 내용은 아래와 같습니다.
